				...
				// 게임 시작
					...
					for (Room r : lobby.getRoomList()) {
						if (r.getRoomNumber() == roomNumber) {
							...
							for (User u : r.getUserList()) {
								r.getTurnQueue().add(u.getUserNumber());  //유저넘버를 큐에 전부 넣는다
							}
							for (User u : r.getUserList()) {
								u.getOutputStream().writeObject(sendMap);
							}
...
//							r.getTurnQueue().add(user.getUserNumber());
					...
					for (Room r : lobby.getRoomList()) {
						if (r.getRoomNumber() == roomNumber) {
							sendMap = new HashMap<>();
							sendMap.put("TURN", r.getTurnQueue().peek()); // 첫번째 데이터를 산출
											//E peek() 해당 큐의 맨 앞에 있는 요소들을 반환함 만약 큐가 비어있다면 null을 반환함
							int turnIndex = -1;
							for (User u : r.getUserList()) {
								if (u.getUserNumber() == r.getTurnQueue().peek()) //유저넘버끼리 비교
									turnIndex = r.getUserList().indexOf(u);	//유저에게 값 -1 를 준다
							}
					...
					for (Room r : lobby.getRoomList()) {
						if (r.getRoomNumber() == roomNumber) {
							sendMap = new HashMap<>();
							sendMap.put("TURN", r.getTurnQueue().peek()); //첫번째 데이터를 산출 peek()
							int turnIndex = -1;
							for (User u : r.getUserList()) {
					...
					for (Room r : lobby.getRoomList()) {
						if (r.getRoomNumber() == roomNumber) {
							if (!r.getUsedWordList().contains(dap) && r.getWordList().contains(dap)) {
								...
								r.getTurnQueue().add(r.getTurnQueue().poll()); //poll 
													//E poll() 해당 큐의 맨 앞에 있는 요소들을 반환하고, 해당 요소를 큐에서 제거함 
													//만약 큐가 비어있다면 null을 반환함
								sendMap = new HashMap<>();
								...
								if (r.getWordList().contains(dap)) { // 이미 사용한 답
									sendMap = new HashMap<>();
									sendMap.put("NOANSWER", true);
									user.getOutputStream().writeObject(sendMap);									
								} else { // 사전에 없는 단어
									sendMap = new HashMap<>();
									sendMap.put("NOANSWER", false);
									user.getOutputStream().writeObject(sendMap);	
								}
							}
							break;
